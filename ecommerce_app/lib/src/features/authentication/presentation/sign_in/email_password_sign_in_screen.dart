...
class _EmailPasswordSignInContentsState
  ...
  // TODO[epic=CALOGERO] Move into state?
  ...
  Future<void> _submit(EmailPasswordSignInState state) async {
    ...
    if (_formKey.currentState!.validate()) {
      final signInController = ref.read(
        emailPasswordSignInControllerProvider(widget.formType).notifier,
      );

      final success = await signInController.submit(email, password);
      if (success) {
              ...
              TextFormField(
                ...
                inputFormatters: <TextInputFormatter>[
                  ValidatorInputFormatter(
                    // Does not allow to use white space
